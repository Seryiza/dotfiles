...
(ert-deftest sz/ewm-launcher-key-waits-for-app-before-creating-tab ()
  (sz/ewm-test-with-tabs
...
(ert-deftest sz/ewm-mapped-app-gets-new-tab-but-floating-window-does-not ()
        ...
        (sz/ewm-test-with-tabs
          ...
          (let ((layout (window-state-get nil t))
            ...
            (cl-letf (((symbol-function 'ewm--surface-target-frame-for-output)
                      ...
                      ((symbol-function 'ewm-display-buffer-floating)
                       (lambda (buffer _alist)
                         (setq floating-shown buffer)
                         (selected-window))))
              (ewm--handle-new-surface '((id . 301) (app . "video-player")))
              (ewm--handle-mapped-surface '((id . 301)))
              (should (= 2 (length (tab-bar-tabs))))
              (should (eq (window-buffer) (gethash 301 ewm--surfaces)))
              (tab-previous)
              (should (equal layout (window-state-get nil t)))
              (tab-next)
              (ewm--handle-new-surface '((id . 302) (app . "dialog")))
              (ewm--handle-mapped-surface '((id . 302) (open_floating . t)))
              (should (eq floating-shown (gethash 302 ewm--surfaces)))
              (should (= 2 (length (tab-bar-tabs))))
              (ewm--handle-close-surface '((id . 302)))
              (should (= 2 (length (tab-bar-tabs)))))))
      (maphash (lambda (_ buffer) (kill-buffer buffer)) ewm--surfaces))))

(ert-deftest sz/ewm-app-close-removes-its-owned-tab ()
  (dolist (scenario '(active background repurposed manually-closed last))
    (ert-info (scenario)
      (let ((ewm--surfaces (make-hash-table :test #'eql))
            (notes (generate-new-buffer "ewm-owned-tab-notes")))
        (unwind-protect
            (sz/ewm-test-with-tabs
              (switch-to-buffer notes)
              (cl-letf (((symbol-function 'ewm--surface-target-frame-for-output)
                         (lambda (_) (selected-frame))))
                (ewm--handle-new-surface '((id . 401) (app . "nemo")))
                (ewm--handle-mapped-surface '((id . 401)))
                (ewm--handle-new-surface '((id . 402) (app . "image-viewer")))
                (ewm--handle-mapped-surface '((id . 402))))
              (let ((image (gethash 402 ewm--surfaces)))
                (tab-bar-rename-tab "Renamed image tab")
                (pcase scenario
                  ('active
                   ;; A close request can be refused or delayed by the client.
                   (let ((ewm--module-mode t) requested)
                     (cl-letf (((symbol-function 'ewm-close)
                                (lambda (id) (setq requested id))))
                       (kill-buffer image))
                     (should (eq requested 402))
                     (should (buffer-live-p image))
                     (should (= 3 (length (tab-bar-tabs))))))
                  ('background
                   (tab-bar-move-tab-to 1)
                   (tab-bar-select-tab 2))
                  ('repurposed (switch-to-buffer notes))
                  ('manually-closed (tab-close))
                  ('last (tab-close-other)))
                (ewm--handle-close-surface '((id . 402)))
                (should-not (buffer-live-p image))
                (should (= (length (tab-bar-tabs)) (if (eq scenario 'last) 1 2)))
                (should-not (seq-some (lambda (tab)
                                        (eq (alist-get 'sz/ewm-owner tab) image))
                                      (tab-bar-tabs)))
                (when (eq scenario 'background)
                  (should (eq (window-buffer) notes)))
                (when (memq scenario '(active repurposed manually-closed))
                  (should (eq (window-buffer) (gethash 401 ewm--surfaces))))
                (should (buffer-live-p notes))
                (should (buffer-live-p (gethash 401 ewm--surfaces)))))
          (maphash (lambda (_ buffer) (kill-buffer buffer)) ewm--surfaces)
          (kill-buffer notes))))))
