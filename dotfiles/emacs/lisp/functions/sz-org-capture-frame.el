...
(defun sz/org-capture--ewm-running-p ()
  "Return non-nil when the current Emacs process hosts EWM."
  (and (fboundp 'ewm-running) (ewm-running)))

(defun sz/org-capture--close-frame (&optional frame)
  "Delete FRAME when it belongs to a dedicated Org Capture session."
  (unless (sz/org-capture--ewm-running-p)
...
(defun sz/org-capture-in-frame ()
  ...
  (if (sz/org-capture--ewm-running-p)
      (org-capture)
