...
pkgs.runCommand "river-session-policy-check"
  ...
  ''
    ...
    grep -F 'alacritty=super+return' "$machi_config"
    grep -F 'swaylock -c 000000=super+escape' "$machi_config"
    grep -F "wmenu-run -i -b -l 10 -f 'Iosevka 14'=super+space" "$machi_config"
    ...
    grep -F 'toggle-river-microphone=XF86HomePage' "$machi_config"
